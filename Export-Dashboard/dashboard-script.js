...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createForwardContractCard(contract) {
        ...
        const bestTradeDay = contract.bestTradeDay || (contract.forecastData && contract.forecastData.best_trade_day) || null;
        const bestTradeDate = bestTradeDay ? new Date(bestTradeDay.date).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
        }) : 'N/A';
        const bestTradePrice = bestTradeDay ? `₹${bestTradeDay.price.toFixed(2)}` : 'N/A';
