...
try {
  ...
  console.log(
    "Firebase initialized successfully with Realtime Database and Firestore"
  );

  // Initialize Importer Firebase for reading orders
  try {
    const existingImporterApp = firebase.apps.find(
      (app) => app.name === "importerApp"
    );
    if (existingImporterApp) {
      importerApp = existingImporterApp;
    } else {
      importerApp = firebase.initializeApp(
        importerFirebaseConfig,
        "importerApp"
      );
    }
    importerDatabase = importerApp.database();
    console.log(
      "Importer Firebase initialized successfully for reading orders"
    );
  } catch (error) {
  ...
  console.warn(
    "Note: Make sure Realtime Database and Firestore are enabled in Firebase Console"
  );
}
...
function loadUserData(user) {
  ...
  if (welcomeName) {
    welcomeName.textContent =
      user.displayName || user.email?.split("@")[0] || "Exporter";
  }
...
async function loadUserProfileData() {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      if (userData.yearEstablished)
        document.getElementById("yearEstablished").value =
          userData.yearEstablished;
      if (userData.businessCategory)
      ...
      // Populate Compliance
      if (userData.iecNumber)
        document.getElementById("iecNumber").value = userData.iecNumber;
      if (userData.gstNumber)
        document.getElementById("gstNumber").value = userData.gstNumber;
      if (userData.panNumber)
        document.getElementById("panNumber").value = userData.panNumber;
      if (userData.cinNumber)
        document.getElementById("cinNumber").value = userData.cinNumber;
      if (userData.dgftRegion)
        document.getElementById("dgftRegion").value = userData.dgftRegion;

      // Populate Products
      if (userData.primaryProduct)
        document.getElementById("primaryProduct").value =
          userData.primaryProduct;
      if (userData.productCategory)
        document.getElementById("productCategory").value =
          userData.productCategory;
      if (userData.hsCode)
        document.getElementById("hsCode").value = userData.hsCode;
      ...
      if (userData.monthlyVolume)
        document.getElementById("monthlyVolume").value = userData.monthlyVolume;
      if (userData.shippingMethod)
      ...
      if (userData.ifscCode)
        document.getElementById("ifscCode").value = userData.ifscCode;
      ...
      if (userData.documents) {
        Object.keys(userData.documents).forEach((docType) => {
          ...
          if (preview && userData.documents[docType]) {
            ...
            const fileName =
              userData.documents[`${docType}FileName`] || `${docType}.pdf`;
            const fileType =
              userData.documents[`${docType}FileType`] || "application/pdf";
...
async function handlePhotoUpload(event) {
  ...
  try {
    ...
    await database
      .ref(`users/${user.uid}/profilePhotoUpdatedAt`)
...
async function saveProductsInfo(event) {
  ...
  const targetCountries = Array.from(targetCountriesSelect.selectedOptions).map(
    (option) => option.value
  );
    ...
    if (statusIcon && statusTitle && statusMessage) {
      ...
      if (status === "verified" || status === "approved") {
        ...
        statusMessage.textContent =
          ...
          (ekycData.rejectionReason || "Not provided") +
      ...
      .onSnapshot(
        (doc) => {
          if (doc.exists) {
            ekycData = doc.data();
            console.log(
              "📡 Real-time eKYC update received:",
              ekycData.ekycStatus
            );
            checkEKYCStatus();
          } else {
...
async function submitEKYC(event) {
  ...
  try {
    ...
    const identityProofType =
      document.getElementById("identityProofType").value;
    const businessProofType =
      document.getElementById("businessProofType").value;
    const bankProofType = document.getElementById("bankProofType").value;

    // Get files
    const identityProofFile =
      document.getElementById("identityProofFile").files[0];
    const businessProofFile =
      document.getElementById("businessProofFile").files[0];
...
async function requestVideoCall() {
  ...
  try {
    ...
    showProfileMessage(
      "Request sent to admin. Waiting for response...",
      "info"
    );
    ...
    showProfileMessage(
      "Failed to request video call. Please try again.",
      "error"
    );
    closeVideoCallModal();
  }
    ...
    .onSnapshot(
      (doc) => {
        ...
        if (data.status === "accepted" && data.agentId) {
          ...
          showProfileMessage(
            "Agent connected! Starting video call...",
            "success"
          );
          ...
          if (data.ekycStatus === "verified") {
            ...
            showProfileMessage(
              "Your eKYC has been verified! Closing video call...",
              "success"
            );
            setTimeout(() => {
              closeVideoCallModal();
...
function listenForAgentAnswer(requestId) {
  ...
  const answerListener = firestore
    .collection("videoCallRequests")
    .doc(requestId)
    .onSnapshot(async (doc) => {
      ...
      if (
        ...
        try {
          console.log(
            "Processing answer from agent, current state:",
          ...
          console.log(
            "Answer remote description set, state:",
            peerConnection.signalingState
          );
        } catch (error) {
...
async function handleProductImageChange(event) {
  ...
  try {
    ...
    showProductMessage(
      "Image processed securely and ready to upload.",
      "success"
    );
  } catch (error) {
...
function formatProductPrice(product) {
  if (product && Number.isFinite(Number(product.priceValue))) {
    ...
    const amount = Number(product.priceValue).toLocaleString(undefined, {
...
async function handleProductSubmit(event) {
  ...
  const payload = {
    ...
    targetMarkets:
      document.getElementById("targetMarkets")?.value?.trim() || "",
...
async function loadProductsFromFirebase() {
  ...
  try {
    ...
    if (badgeEl) {
      badgeEl.textContent = `${products.length} ${
...
async function removeProductFromCatalog(productId) {
  ...
  try {
    ...
    console.warn(
      "Product catalog removal skipped:",
      error?.code || error?.message || error
    );
  }
}
...
function buildProductCard(product, ids = {}) {
  ...
  const description = escapeHTML(
    product.description || "No description shared."
  );
  const price = formatProductPrice(product);
  const incoterm = escapeHTML(
    product.incoterm ? product.incoterm.toUpperCase() : "Flexible"
  );
  ...
  const packaging = escapeHTML(
    product.packagingDetails || "Standard export-ready packaging available."
  );
  const compliance = escapeHTML(
    product.complianceNotes || "Complies with buyer / destination requirements."
  );
        ...
        <div class="product-chip-group">
            ...
            <div class="product-chip-wrapper">
                ${marketItems
                  .map(
                    (item) =>
                      `<span class="product-chip">${escapeHTML(item)}</span>`
                  )
                  .join("")}
            </div>
        </div>
        ...
        <div class="product-chip-group">
            ...
            <div class="product-chip-wrapper">
                ${certificationItems
                  .map(
                    (item) =>
                      `<span class="product-chip neutral">${escapeHTML(
                        item
                      )}</span>`
                  )
                  .join("")}
            ...
            <div id="${contentId}" class="product-card-content" hidden>
                ...
                <div class="product-card-footer">
                    ...
                    <div class="product-card-actions">
                        <button type="button" class="product-card-action delete" onclick="deleteProduct('${
                          product.id
                        }')">
                            Delete
                        </button>
...
const supportFaqData = [
  {
    ...
    answer:
      "Open the Documents tab and click “Complete eKYC”. Upload your Aadhaar/PAN/Passport along with business and bank proofs, then submit the video verification request.",
  },
  {
    question: "Which documents are required for Indian exporters?",
    answer:
      "Typically IEC certificate, GST certificate, PAN, address proof, bank letter/cancelled cheque, and any licenses specific to your product category.",
  },
  {
    question: "Which documents are required for international exporters?",
    answer:
      "Common documents include Certificate of Origin, commercial invoice, packing list, inspection certificates, insurance policy, and destination-specific compliance certificates.",
  },
  {
    question: "How to upload ID proof or business proof?",
    answer:
      "Go to the Profile modal → Compliance tab and upload the respective files. They are stored securely in Firebase with Base64 encoding.",
  },
  {
    question: "What is currency protection / forward contract?",
    answer:
      "Forward Contracts lock in an exchange rate for a future date so your export receivables aren’t impacted by FX fluctuations. See the Forward Contracts tab for details.",
  },
  {
    question: "How does QR shipment documentation work?",
    answer:
      "All shipment documents are digitised, stored against your account, and can be shared via QR codes with logistics partners for faster verification.",
  },
  {
    question: "How to track my shipment?",
    answer:
      "Use the Dashboard or Insurance tab to see live shipment milestones once tracking IDs are added. We send alerts for exceptions automatically.",
  },
  {
    question: "How to contact support?",
    answer:
      "Use this chat window or email support@globalguardexports.com. When an admin is online you can start a live session.",
  },
];
...
const supportBotKnowledge = [
  {
    ...
    answer:
      "You can complete eKYC from the Documents tab. Upload Aadhaar/PAN/Passport plus business/bank proofs, submit for review, and request a video verification slot.",
  },
  {
    keywords: [
      "document",
    ...
    answer:
      "All uploaded documents are stored securely in Firebase Realtime Database and appear inside your Document Library. You can preview, download, or delete them anytime.",
  },
  {
    keywords: ["shipment", "track", "tracking"],
    answer:
      "Shipment tracking lives in the Dashboard insights and Insurance tab. Add your tracking IDs and we’ll surface live milestones plus risk alerts.",
  },
  {
    keywords: ["forward contract", "currency", "fx", "hedge"],
    answer:
      "A Forward Contract locks in an exchange rate for a future payout. Use the Forward Contracts tab to simulate rates and share details with your bank.",
  },
  {
    keywords: ["insurance", "risk", "coverage"],
    answer:
      "Our Insurance panel compares marine policies, coverage adequacy, and appetite for your corridor. Complete the shipment questionnaire to get matched policies instantly.",
  },
  {
    keywords: ["support", "human", "agent"],
    answer:
      "I am always available. If an admin agent is online you’ll also see “Chat with Admin (Live)” in this window for real-time help.",
  },
];
...
function updateSupportAdminStatus(isOnline) {
  ...
  if (statusIndicator && statusLabel) {
    ...
    statusLabel.textContent = isOnline
      ? "Admin Online · Live chat available"
      : "Admin Offline · Bot assisting";
    if (dot) {
...
function syncDocumentRequirementsContext(doc) {
  ...
  if (
    ...
    getCountryDataById(hintedCountry)
  ) {
    countrySelect.value = hintedCountry;
    updated = true;
  }
...
function createDocumentRecord({
  ...
  const { base64, mimeType: extractedMime } = extractBase64Components(
    rawData,
    fileType
  );
...
function collectRealtimeDocuments(userData = {}) {
  ...
  Object.keys(userData.documents).forEach((docKey) => {
    ...
    const normalizedValue =
      typeof rawValue === "object" && rawValue.data ? rawValue.data : rawValue;
    const fileName =
...
function initializeDocumentsModule() {
  ...
  if (documentSearch) {
    documentSearch.addEventListener(
      "input",
      debounce(handleDocumentSearch, 300)
    );
  }
...
async function loadDocumentsFromFirebase() {
  ...
  try {
    const realtimePromise = database
      ? database.ref(`users/${user.uid}`).once("value")
      : Promise.resolve(null);
    const firestorePromise = firestore
      ? firestore.collection("ekyc").doc(user.uid).get()
      : Promise.resolve(null);
    const [snapshot, ekycDoc] = await Promise.all([
      realtimePromise,
      firestorePromise,
    ]);
...
function renderDocuments(documents) {
  ...
  gridEl.innerHTML = documents
    .map((doc) => {
      ...
      const previewHTML =
        isImageMimeType(doc.mimeType) && doc.data
...
function handleDocumentSearch(event) {
  ...
  const filtered = documentsCache.filter(
    (doc) =>
...
function openDocumentDetail(docId) {
  ...
  if (dateEl)
    dateEl.textContent = new Date(doc.uploadedAt).toLocaleDateString();
  if (fileTypeEl)
    fileTypeEl.textContent = getFileExtension(doc.mimeType).toUpperCase();
  ...
  return `
    ...
    item.name
            ...
            <div style="flex: 1; display: flex; flex-direction: column; gap: 0.5rem;">
                ...
                ${
                  item.exporterName
                    ? `<div style="color: #666; font-size: 0.9rem;">From: ${item.exporterName}</div>`
                    : ""
                }
                <div style="display: flex; align-items: center; gap: 1rem; margin-top: auto;">
                    <div style="display: flex; align-items: center; gap: 0.5rem; background: white; border: 2px solid #e0e0e0; border-radius: 6px; padding: 0.25rem 0.5rem;">
                        <button onclick="updateExporterCartQuantity('${
                          item.id
                    ...
                    <button onclick="removeExporterCartItem('${
                      item.id
      ...
      .map(
        (item) => `
            <div class="order-summary-item">
                <span>${item.name || "Unnamed Product"} x ${
          item.quantity
        }</span>
                <span>${currency} ${(
...
async function handleCheckoutSubmit(event) {
  ...
  // Calculate total
  const total = currentCheckoutCart.reduce(
...
async function saveOrderToFirebase(orderData) {
  ...
  for (const [exporterId, exporterOrder] of Object.entries(ordersByExporter)) {
    try {
      ...
      console.log(
        "Order saved to exporter incomingOrders:",
        exporterId,
        "Order key:",
        orderKey
      );
...
async function loadOrderHistory() {
  ...
  orderHistoryList.innerHTML =
    '<div style="text-align: center; padding: 2rem;"><div class="loading-spinner"></div><p>Loading orders...</p></div>';
  ...
  try {
    ...
    if (!ordersData) {
      orderHistoryList.innerHTML =
        '<div style="text-align: center; padding: 3rem; color: #666;"><p>No orders yet.</p></div>';
      return;
    }
    ...
    orderHistoryList.innerHTML = orders
      .map((order) => createOrderHistoryItemHTML(order))
      .join("");
  } catch (error) {
    console.error("Error loading order history:", error);
    orderHistoryList.innerHTML =
      '<div style="text-align: center; padding: 3rem; color: #d32f2f;"><p>Error loading orders. Please try again.</p></div>';
  }
}
  ...
  return `
        <div class="order-history-item">
            <div class="order-history-header">
                ...
                <div>
                    <span class="order-status-badge ${paymentStatus}">${
    paymentStatus === "paid" ? "Paid" : "Payment Pending"
            ...
            <div class="order-items">
                ${order.items
                  .map((item) => {
                    const imageSrc = item.imageBase64
                      ? `data:${item.imageFileType || "image/jpeg"};base64,${
                          item.imageBase64
                        }`
                    ...
                    return `
                      ...
                      item.name
                            ...
                            <div class="order-item-details">
                                ...
                                <div class="order-item-meta">Quantity: ${
                                  item.quantity
                                } × ${order.currency} ${item.price}</div>
                            </div>
                        </div>
                    `;
                  })
...
async function loadIncomingOrders() {
  ...
  if (!user || !database) {
    console.warn(
      "Cannot load incoming orders: User not authenticated or database not initialized"
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      "Fetching from exporter database: users/" + user.uid + "/incomingOrders"
    );
    ...
    if (!ordersData || Object.keys(ordersData).length === 0) {
      ...
      incomingOrdersList.innerHTML =
        '<div style="text-align: center; padding: 3rem; color: #666;"><div style="font-size: 3rem; margin-bottom: 1rem;">📦</div><p>No incoming orders yet.</p><p style="font-size: 0.9rem; margin-top: 0.5rem; color: #9ca3af;">Orders placed for your products will appear here.</p></div>';
      return;
    }
    ...
    if (orders.length === 0) {
      incomingOrdersList.innerHTML =
        '<div style="text-align: center; padding: 3rem; color: #666;"><div style="font-size: 3rem; margin-bottom: 1rem;">📦</div><p>No incoming orders yet.</p></div>';
      return;
    }

    incomingOrdersList.innerHTML = orders
      .map((order) => createIncomingOrderCardHTML(order))
      .join("");
    ...
    incomingOrdersList.innerHTML =
      '<div style="text-align: center; padding: 3rem; color: #d32f2f;"><p>Error loading orders. Please try again.</p><p style="font-size: 0.85rem; margin-top: 0.5rem; color: #9ca3af;">' +
      error.message +
      "</p></div>";
  }
}
...
function createIncomingOrderCardHTML(order) {
  ...
  return `
        <div class="incoming-order-card">
            <div class="incoming-order-header">
                <div>
                    ...
                    <div class="incoming-order-buyer">From: ${
                      order.buyerName || order.buyerEmail || "Unknown"
                    }</div>
    ...
    order.shipping.zipCode
            ...
            <div class="order-document-section" id="orderDocSection_${
              order.id
            }" style="margin-top: 1.5rem; padding: 1.5rem; background: #f0f9ff; border-radius: 12px; border: 2px solid #0ea5e9;">
                ...
                <div class="order-document-upload-area" style="margin-bottom: 1rem;">
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        <div>
                            ...
                            <textarea id="orderMessage_${
                              order.id
                            }" placeholder="Add a message with your documents..." rows="3" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 8px; font-family: inherit; resize: vertical;"></textarea>
                        </div>
                        <div>
                            <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #334155;">Upload Documents/Images/Videos</label>
                            
                            <!-- Drag and Drop Area -->
                            <div id="orderDocDropZone_${
                              order.id
                            }" class="order-doc-drop-zone" 
                                 ...
                                 ondrop="handleOrderDocumentDrop(event, '${
                                   order.id
                                 }')" 
                                 ondragover="handleOrderDocumentDragOver(event)" 
                                 ondragleave="handleOrderDocumentDragLeave(event, '${
                                   order.id
                                 }')"
                                 onclick="document.getElementById('orderDocument_${
                                   order.id
                                 }').click()">
                            ...
                            <!-- Hidden File Input -->
                            <input type="file" id="orderDocument_${
                              order.id
                            }" multiple accept="image/*,video/*,.pdf,.doc,.docx" onchange="handleOrderDocumentChange('${
                    order.id
                  }')" style="display: none;">
                            
                            <div id="orderDocPreview_${
                              order.id
                ...
                <div style="display: flex; gap: 0.75rem;">
                    <button onclick="sendOrderUpdate('${order.id}', '${
                    order.buyerId || ""
                  }', '${
                    order.orderId || ""
                  }')" class="btn-primary" style="padding: 0.75rem 1.5rem; border-radius: 8px; border: none; background: #0ea5e9; color: white; cursor: pointer; font-weight: 500; flex: 1;">
                        ...
                        ${order.orderUpdates
                          .map(
                            ...
                            <div style="padding: 0.75rem; background: white; border-radius: 8px; border: 1px solid #e2e8f0;">
                                ...
                                ${
                                  update.message
                                    ? `<p style="color: #334155; margin-bottom: 0.5rem;">${escapeHtml(
                                        update.message
                                      )}</p>`
                                    : ""
                                }
                                    ...
                                    <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 0.5rem;">
                                        ${update.documents
                                          .map(
                                            ...
                                            }" target="_blank" style="display: inline-flex; align-items: center; gap: 0.25rem; padding: 0.25rem 0.5rem; background: #f1f5f9; border-radius: 6px; text-decoration: none; color: #0ea5e9; font-size: 0.85rem;">
            ...
            <div class="incoming-order-actions" style="margin-top: 1.5rem;">
                ${
                  orderStatus !== "processing"
                    ...
                    <button class="btn-primary" onclick="updateOrderStatus('${
                      order.id
                    }', 'processing', '${order.importerId || ""}', '${
                        order.originalOrderKey || ""
                      }')" style="padding: 0.5rem 1rem; border-radius: 6px; border: none; background: #667eea; color: white; cursor: pointer;">
                        Mark as Processing
                    </button>
                `
                    : ""
                ...
                ${
                  orderStatus !== "completed"
                    ? `
                    <button class="btn-secondary" onclick="updateOrderStatus('${
                      order.id
                    }', 'completed', '${order.importerId || ""}', '${
                        order.originalOrderKey || ""
                      }')" style="padding: 0.5rem 1rem; border-radius: 6px; border: 1px solid #e5e7eb; background: white; cursor: pointer;">
                        Mark as Completed
                    </button>
                `
                    : ""
...
async function updateOrderStatus(
  ...
  try {
    ...
    const orderRef = database.ref(
      `users/${user.uid}/incomingOrders/${orderId}`
    );
    ...
    if (order.buyerId && importerDatabase) {
      try {
        ...
        const importerOrdersSnapshot = await importerDatabase
          .ref(importerOrdersPath)
          .once("value");
        ...
        if (importerOrders) {
          ...
          if (Array.isArray(importerOrders)) {
            const orderIndex = importerOrders.findIndex(
              (o) =>
                o && (o.orderId === order.orderId || o.id === order.orderId)
            );
            if (orderIndex !== -1) {
              await importerDatabase
                .ref(`${importerOrdersPath}/${orderIndex}`)
                .update({
                  status: newStatus,
                  updatedAt: new Date().toISOString(),
                });
            ...
            for (const [key, importerOrder] of Object.entries(importerOrders)) {
              if (
                importerOrder &&
                (importerOrder.orderId === order.orderId ||
                  importerOrder.id === order.orderId)
              ) {
                await importerDatabase
                  .ref(`${importerOrdersPath}/${key}`)
                  .update({
                    status: newStatus,
                    updatedAt: new Date().toISOString(),
                  });
        ...
        console.warn(
          "Could not update importer database (permission issue or not accessible):",
          importerError
        );
...
function handleOrderDocumentChange(orderId) {
  ...
  Array.from(files).forEach((file, index) => {
    ...
    fileItem.style.cssText =
      "display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem; background: white; border-radius: 8px; border: 1px solid #e2e8f0; box-shadow: 0 1px 2px rgba(0,0,0,0.05);";
    fileItem.innerHTML = `
            ${iconSvg}
            <span style="flex: 1; font-size: 0.9rem; color: #334155; font-weight: 500;">${escapeHtml(
              file.name
            )}</span>
...
async function sendOrderUpdate(orderId, buyerId, orderOrderId) {
  ...
  const sendBtn =
    event?.target ||
    document.querySelector(`button[onclick*="sendOrderUpdate('${orderId}'"]`);
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (!isValidType) {
        alert(
          `File "${file.name}" is not a supported type. Please upload images, videos, PDF, or Word documents.`
        );
        continue;
      }
    ...
    if (importerDatabase) {
      try {
        ...
        await importerDatabase
          .ref(`importers/${buyerId}/notifications`)
          .push(notificationData);
        console.log("Notification sent to importer:", buyerId);
      } catch (notifError) {
        console.warn(
          "Could not send notification to importer (permission issue):",
          notifError
        );
    ...
    if (importerDatabase && orderOrderId) {
      try {
        ...
        if (importerOrders) {
          if (Array.isArray(importerOrders)) {
            const orderIndex = importerOrders.findIndex(
              (o) => o && (o.orderId === orderOrderId || o.id === orderOrderId)
            );
            if (orderIndex !== -1) {
            ...
            for (const [key, importerOrder] of Object.entries(importerOrders)) {
              if (
                importerOrder &&
                (importerOrder.orderId === orderOrderId ||
                  importerOrder.id === orderOrderId)
              ) {
                const importerOrderUpdates = importerOrder.orderUpdates || [];
                importerOrderUpdates.push(updateData);
                await importerDatabase
        ...
        console.warn(
          "Could not update importer order (permission issue):",
          updateError
        );
        // Continue anyway
      }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const incomingOrdersTab = document.querySelector(
    '[data-tab="incoming-orders"]'
  );
...
async function loadNotifications() {
  ...
  try {
    ...
    notificationList.innerHTML = notifications
      .map((notification) => createNotificationItemHTML(notification))
      .join("");
  } catch (error) {
    console.error("Error loading notifications:", error);
    notificationList.innerHTML =
      '<div class="notification-empty"><p>Error loading notifications</p></div>';
  }
}
...
function listenForNotifications() {
  ...
  if (notificationListener) {
    database
      .ref(`users/${user.uid}/notifications`)
      .off("child_added", notificationListener);
  }
  ...
  database
    .ref(`users/${user.uid}/notifications`)
    .on("child_added", notificationListener);
}
...
function createNotificationItemHTML(notification) {
  ...
  if (notification.type === "new_order") {
    message = `New order #${notification.orderId} from ${
  ...
  const readIndicator = notification.read
    ? ""
    : '<span class="notification-unread-indicator"></span>';

  return `
        <div class="notification-item ${readClass}" data-notification-id="${
    notification.id
  }" onclick="handleNotificationClick('${notification.id}', '${
    notification.type
  }', '${notification.orderKey || ""}')">
  ...
  try {
    ...
    Object.keys(notifications).forEach((id) => {
      if (!notifications[id].read) {
        ...
        updates[`users/${user.uid}/notifications/${id}/readAt`] =
...
document.addEventListener("DOMContentLoaded", function () {
            ...
            <div class="forward-contract-result-block">
                ...
                <div class="forward-contract-result-content">
                    <div style="margin-bottom: 20px;">
                        ...
                        <p style="font-size: 1.1rem; margin: 5px 0;"><strong>Price:</strong> ₹${forecast.best_trade_day.price.toFixed(
                          2
                        )}</p>
  ...
  async function generatePDF(data) {
    ...
    doc.setFontSize(18);
    doc.text("FORWARD CONTRACT NOTE", pageWidth / 2, y + 8, { align: "center" });
    
    doc.setFontSize(9);
    doc.setFont("Helvetica", "normal");
    doc.text("FXcgo Pvt. Ltd.", pageWidth / 2, y + 14, { align: "center" });
    ...
    // Load signature/seal image if available
    ...
    doc.setFont("Helvetica", "normal");
    doc.setFontSize(10);
    doc.text("Pritish Kumar", detailsX, detailY + 6);
    doc.text("Treasury Operations", detailsX, detailY + 12);
    doc.text("FXcgo Pvt. Ltd.", detailsX, detailY + 18);
    ...
    if (auth && database && auth.currentUser) {
      ...
      try {
        await database
          .ref(`users/${user.uid}/forwardContracts/${contractId}`)
          .set(contractData);
  ...
  function createForwardContractCard(contract) {
    ...
    const currentRate = contract.currentRate
      ...
      : "N/A";
    const bestTradeDay =
      contract.bestTradeDay ||
      (contract.forecastData && contract.forecastData.best_trade_day) ||
      null;
    const bestTradeDate = bestTradeDay
      ? new Date(bestTradeDay.date).toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        })
      : "N/A";
    const bestTradePrice = bestTradeDay
      ? `₹${bestTradeDay.price.toFixed(2)}`
      : "N/A";
    ...
    card.innerHTML = `
            ...
            <div class="forward-contract-card-actions">
                <button class="forward-contract-action-btn forward-contract-action-btn-primary" onclick="downloadForwardContractPDF('${
                  contract.id
                }')">
                    <span>📄</span>
                    <span>Download PDF</span>
                </button>
                <button class="forward-contract-action-btn forward-contract-action-btn-secondary" onclick="viewForwardContractDetails('${
                  contract.id
                }')">
                    <span>👁️</span>
  ...
  async function downloadForwardContractPDF(contractId) {
    ...
    try {
      const contractRef = database.ref(
        `users/${user.uid}/forwardContracts/${contractId}`
      );
  ...
  async function viewForwardContractDetails(contractId) {
    ...
    try {
      ...
      const bestTradePrice = bestTradeDay
        ? `₹${bestTradeDay.price.toFixed(2)}`
        : "N/A";
      const bestTradeReason = bestTradeDay ? bestTradeDay.reason : "N/A";
