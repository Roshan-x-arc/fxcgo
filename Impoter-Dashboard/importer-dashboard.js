...
(function () {
  ...
  function initFirebase() {
    try {
      ...
      importerApp =
        existing ||
        firebase.initializeApp(importerFirebaseConfig, "importerApp");
    ...
    try {
      const existingExporter = firebase.apps.find(
        (app) => app.name === "exporterApp"
      );
      if (existingExporter) {
        exporterApp = existingExporter;
      } else {
        exporterApp = firebase.initializeApp(
          exporterFirebaseConfig,
          "exporterApp"
        );
      }
  ...
  function activateSection(target, options = {}) {
    ...
    document.querySelectorAll(".nav-link[data-tab]").forEach((link) => {
      ...
      if (isActive && link.id) {
        document
          .querySelector('.nav-list[role="tablist"]')
          ?.setAttribute("aria-activedescendant", link.id);
      }
    });
    ...
    if (pageTitle) {
      pageTitle.textContent =
        sectionTitleMap[normalizedTab] || sectionTitleMap[DEFAULT_TAB];
    }
    if (syncHash) {
  ...
  function bindRealtime(path, handler) {
    ...
    const errorHandler = (error) => {
      ...
      if (
        ...
        console.warn(
          `Permission denied for ${path}. Please configure Firebase security rules.`
        );
      }
  ...
  function renderSummaryCards(data = {}) {
    ...
    document.getElementById("summaryActiveShipments").textContent =
      data.activeShipments ?? 0;
    document.getElementById("summaryPendingPayments").textContent =
      data.pendingPayments ?? 0;
    document.getElementById("summaryPendingVerification").textContent =
      data.pendingVerification ?? 0;
    document.getElementById("summaryDeliveredShipments").textContent =
  ...
  function renderDocumentsSection(data = {}) {
    ...
    const template = documents.length
      ? documents
  ...
  function renderSupportModule(data = {}) {
    ...
    if (chatLog) {
      ...
      const messages = convertToArray(
        data && data.messages ? data.messages : null
    ...
    if (faqList) {
      ...
      const topics = faqs.length
        ? faqs.map((item) => item.title || item.question)
        : defaultFaqs;
    ...
    notifications
      ...
      .forEach((notification) => {
        ...
        if (notification.type === "order_update") {
          ...
          const hasDocs =
            notification.hasDocuments && notification.documentCount > 0;
          ...
          notificationHTML = `
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                            <div style="flex: 1;">
                                ...
                                <p style="margin: 0.5rem 0; color: #334155;">${escapeHtml(
                                  notification.message ||
                                    "New update for your order"
                                )}</p>
                                ${
                                  notification.orderId
                                    ? `<p style="margin: 0.25rem 0; font-size: 0.85rem; color: #64748b;">Order #${escapeHtml(
                                        notification.orderId
                                      )}</p>`
                                    : ""
                                }
                                ${
                                  notification.exporterName
                                    ? `<p style="margin: 0.25rem 0; font-size: 0.85rem; color: #64748b;">From: ${escapeHtml(
                                        notification.exporterName
                                      )}</p>`
                                    : ""
                                }
                            </div>
                            <small style="color: #94a3b8; white-space: nowrap; margin-left: 1rem;">${formatDate(
                              notification.createdAt
                            )}</small>
                        </div>
                        ${
                          updateData.message
                            ? `<div style="padding: 0.75rem; background: white; border-radius: 8px; margin: 0.5rem 0; border-left: 3px solid #0ea5e9;"><p style="margin: 0; color: #334155;">${escapeHtml(
                                updateData.message
                              )}</p></div>`
                            : ""
                        }
                        ${
                          ...
                          updateData.documents.length > 0
                            ...
                            <div style="margin-top: 0.75rem;">
                                <p style="font-weight: 500; margin-bottom: 0.5rem; color: #334155; font-size: 0.9rem;">📎 Documents (${
                                  updateData.documents.length
                                }):</p>
                                <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
                                    ${updateData.documents
                                      .map((doc) => {
                                        const docUrl = doc.base64
                                          ? `data:${
                                              doc.fileType ||
                                          ...
                                          : "#";
                                        return `
                                            <a href="${docUrl}" download="${escapeHtml(
                                          doc.fileName || "document"
                                        )}" target="_blank" style="display: inline-flex; align-items: center; gap: 0.25rem; padding: 0.5rem 0.75rem; background: #0ea5e9; color: white; border-radius: 6px; text-decoration: none; font-size: 0.85rem; transition: background 0.2s; hover:background: #0284c7;">
          ...
          notificationHTML = `
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <small style="color: #94a3b8;">${formatDate(
                              notification.createdAt
                            )}</small>
                        </div>
                    `;
  ...
  function handleSupportMessageSubmit(event) {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/support/messages`)
      .push(payload)
      .then(() => {
        form.reset();
  ...
  function attachEvents() {
    ...
    document
      .getElementById("supportMessageForm")
      ?.addEventListener("submit", handleSupportMessageSubmit);
    document
      .getElementById("documentsViewBtn")
      ?.addEventListener("click", handleDocumentAction);
    document
      .getElementById("documentsDownloadBtn")
      ?.addEventListener("click", handleDocumentAction);

    document
      .getElementById("refreshImporterData")
    ...
    document
      .getElementById("importerLogoutBtn")
    ...
    importerAuth.onAuthStateChanged((user) => {
      ...
      if (usernameEl) {
        usernameEl.textContent =
          user.displayName || user.email || "Importer Workspace";
      }
    ...
    try {
      ...
      ekycRef
        .once("value", (snapshot) => {
          ...
          const ekycStatusCard = document.getElementById(
            "importerEkycStatusCard"
          );
          const videoCallRequestCard = document.getElementById(
            "importerVideoCallRequestCard"
          );
          ...
          if (!snapshot.exists()) {
            ...
            if (videoCallRequestCard)
              videoCallRequestCard.style.display = "none";
            return;
          }
          ...
          const statusTitle = document.getElementById(
            "importerEkycStatusTitle"
          );
          const statusMessage = document.getElementById(
            "importerEkycStatusMessage"
          );
          ...
          if (statusIcon && statusTitle && statusMessage) {
            ...
            if (status === "verified" || status === "approved") {
              ...
              statusMessage.textContent =
                "Your eKYC has been verified successfully. You can now access all features.";
              if (videoCallRequestCard)
                videoCallRequestCard.style.display = "none";
              ...
              statusMessage.textContent =
                "Your eKYC verification was rejected. Reason: " +
                (importerEkycData.rejectionReason || "Not provided") +
                ". Please contact support to resubmit.";
              if (videoCallRequestCard)
                videoCallRequestCard.style.display = "none";
            } else {
              statusIcon.textContent = "📹";
              ...
              statusTitle.textContent =
                "eKYC Submitted - Awaiting Verification";
              statusMessage.textContent =
                "Your documents have been received. Complete your verification with a video call to finish the process.";
              if (videoCallRequestCard)
        ...
        .catch((error) => {
          ...
          if (
            ...
            console.warn(
              "Permission denied. Please configure Firebase security rules."
            );
  ...
  async function openImporterEKYCModal() {
    ...
    try {
      if (importerDatabase) {
        const userRef = importerDatabase.ref(
          `${currentUserBasePath}/profile/address`
        );
  ...
  function populateImporterEKYCForm() {
    const isIndia =
      importerUserCountry && importerUserCountry.toLowerCase() === "india";
    ...
    if (businessSelect) {
      ...
      const options = isIndia
        ? INDIAN_BUSINESS_PROOFS
        : INTERNATIONAL_BUSINESS_PROOFS;
      options.forEach((option) => {
        const opt = document.createElement("option");
        opt.value = option.value;
        opt.textContent = option.label;
  ...
  async function startImporterCamera() {
    try {
      ...
      const previewContainer = document.getElementById(
        "importerCameraPreviewContainer"
      );
  ...
  function handleImporterFilePreview(event, type) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      showImporterEKYCMessage(
        `${type} file size should be less than 10MB`,
        "error"
      );
      event.target.value = "";
      preview.innerHTML = "";
  ...
  function clearImporterFilePreviews() {
    const previews = [
      "importerIdentityProofPreview",
      "importerBusinessProofPreview",
      "importerBankProofPreview",
      "importerSelfiePreview",
    ];
    previews.forEach((id) => {
  ...
  async function submitImporterEKYC(event) {
    ...
    try {
      ...
      if (snapshot.exists() && snapshot.val().ekycCompleted === true) {
        showImporterEKYCMessage(
          "You have already completed your eKYC",
          "error"
        );
        return;
      }
    ...
    try {
      const identityProofType = document.getElementById(
        "importerIdentityProofType"
      ).value;
      const businessProofType = document.getElementById(
        "importerBusinessProofType"
      ).value;
      const bankProofType = document.getElementById(
        "importerBankProofType"
      ).value;
      const identityProofFile = document.getElementById(
        "importerIdentityProofFile"
      ).files[0];
      const businessProofFile = document.getElementById(
        "importerBusinessProofFile"
      ).files[0];
      const bankProofFile = document.getElementById("importerBankProofFile")
        .files[0];
      ...
      if (!importerSelfieDataUrl) {
        showImporterEKYCMessage(
          "Please capture a live selfie before submitting",
          "error"
        );
        if (submitBtn) {
          submitBtn.disabled = false;
      ...
      const ekycData = {
        ...
        userName:
          document.getElementById("importerFullName")?.value?.trim() ||
      ...
      await importerDatabase
        .ref(`${currentUserBasePath}/ekyc`)
        .set(ekycData)
        .catch((error) => {
          ...
          if (
            ...
            showImporterEKYCMessage(
              "Permission denied. Please configure Firebase security rules to allow authenticated users to write their own data.",
              "error"
            );
          } else {
            showImporterEKYCMessage(
              "Failed to submit eKYC. Please try again.",
              "error"
            );
          }
          throw error;
      ...
      showImporterEKYCMessage(
        "Your eKYC has been submitted successfully. You can now request a video call for verification.",
        "success"
      );
  ...
  async function requestImporterVideoCall() {
    ...
    if (modal) {
      ...
      modal.setAttribute("aria-hidden", "false");
      document.body.style.overflow = "hidden";
      document.getElementById("importerWaitingForAgent").style.display =
        "block";
      document.getElementById("importerVideoCallInterface").style.display =
        "none";
    ...
    try {
      ...
      const requestRef = importerDatabase
        .ref(`${currentUserBasePath}/videoCallRequests`)
        .push(requestData);
  ...
  function listenForImporterAgentAcceptance(requestId) {
    if (!importerDatabase) return;
    const requestRef = importerDatabase.ref(
      `${currentUserBasePath}/videoCallRequests/${requestId}`
    );
    requestRef.on("value", (snapshot) => {
      ...
      if (data.status === "accepted" && data.agentId) {
        ...
        closeImporterVideoCallModal();
        showToast(
          "Your verification request was rejected. Please try again later."
        );
    ...
    try {
      ...
      importerPeerConnection.onicecandidate = (event) => {
        if (event.candidate) {
          ...
          importerDatabase
            .ref(`${currentUserBasePath}/videoCallRequests/${requestId}`)
            .update({
      ...
      requestRef.on("value", async (snapshot) => {
        ...
        if (
          data.answer &&
          importerPeerConnection &&
          importerPeerConnection.signalingState === "have-local-offer"
        ) {
          try {
            await importerPeerConnection.setRemoteDescription(
        ...
        if (data.agentIceCandidate && importerPeerConnection) {
          try {
            await importerPeerConnection.addIceCandidate(
              new RTCIceCandidate(data.agentIceCandidate)
            );
          } catch (error) {
  ...
  function closeImporterVideoCallModal() {
    ...
    if (importerVideoCallListener && importerDatabase) {
      const requestRef = importerDatabase.ref(
        `${currentUserBasePath}/videoCallRequests/${importerVideoCallRequestId}`
      );
  ...
  function switchImporterProfileTab(sectionName) {
    const sections = document.querySelectorAll(
      "#importerProfileModal .profile-section"
    );
    ...
    const tabs = document.querySelectorAll(
      "#importerProfileModal .profile-tab"
    );
    tabs.forEach((tab) => {
  ...
  function loadImporterProfileData() {
    ...
    profileRef
      .once("value", (snapshot) => {
        const data = snapshot.val() || {};
        if (data.basic) {
          document.getElementById("importerFullName").value =
            data.basic.fullName || "";
          document.getElementById("importerEmail").value =
            currentUser.email || "";
          document.getElementById("importerPhoneNumber").value =
            data.basic.phoneNumber || "";
          document.getElementById("importerAlternatePhone").value =
            data.basic.alternatePhone || "";
          if (data.basic.profilePhoto) {
            const img = document.getElementById("importerPhotoPreviewImg");
            img.src = data.basic.profilePhoto;
            img.style.display = "block";
            document.getElementById("importerPhotoPreviewText").style.display =
              "none";
          }
        }
        if (data.business) {
          document.getElementById("importerCompanyName").value =
            data.business.companyName || "";
          document.getElementById("importerBusinessType").value =
            data.business.businessType || "";
          document.getElementById("importerYearEstablished").value =
            data.business.yearEstablished || "";
          document.getElementById("importerBusinessCategory").value =
            data.business.businessCategory || "";
          document.getElementById("importerCompanyWebsite").value =
            data.business.companyWebsite || "";
          document.getElementById("importerCompanyDescription").value =
            data.business.companyDescription || "";
        }
        if (data.compliance) {
          document.getElementById("importerIecNumber").value =
            data.compliance.iecNumber || "";
          document.getElementById("importerGstNumber").value =
            data.compliance.gstNumber || "";
          document.getElementById("importerPanNumber").value =
            data.compliance.panNumber || "";
          document.getElementById("importerCinNumber").value =
            data.compliance.cinNumber || "";
          document.getElementById("importerDgftRegion").value =
            data.compliance.dgftRegion || "";
        }
        if (data.import) {
          document.getElementById("importerPrimaryProduct").value =
            data.import.primaryProduct || "";
          document.getElementById("importerProductCategory").value =
            data.import.productCategory || "";
          document.getElementById("importerHsCode").value =
            data.import.hsCode || "";
          document.getElementById("importerMonthlyVolume").value =
            data.import.monthlyVolume || "";
          document.getElementById("importerShippingMethod").value =
            data.import.shippingMethod || "";
        }
        if (data.address) {
          document.getElementById("importerBusinessAddress").value =
            data.address.businessAddress || "";
          document.getElementById("importerCity").value =
            data.address.city || "";
          document.getElementById("importerState").value =
            data.address.state || "";
          document.getElementById("importerCountry").value =
            data.address.country || "";
          document.getElementById("importerPincode").value =
            data.address.pincode || "";
        }
        if (data.banking) {
          document.getElementById("importerBankName").value =
            data.banking.bankName || "";
          document.getElementById("importerAccountHolderName").value =
            data.banking.accountHolderName || "";
          document.getElementById("importerAccountNumber").value =
            data.banking.accountNumber || "";
          document.getElementById("importerIfscCode").value =
            data.banking.ifscCode || "";
          document.getElementById("importerPaymentMethod").value =
            data.banking.paymentMethod || "";
        }
      })
      .catch((error) => {
  ...
  function saveImporterBasicInfo(event) {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/profile/basic`)
      .update(data)
  ...
  function saveImporterBusinessInfo(event) {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/profile/business`)
      .update(data)
  ...
  function saveImporterComplianceInfo(event) {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/profile/compliance`)
      .update(data)
  ...
  function saveImporterImportInfo(event) {
    ...
    const selectedCountries = Array.from(
      form.sourceCountries.selectedOptions
    ).map((opt) => opt.value);
    const data = {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/profile/import`)
      .update(data)
  ...
  function saveImporterAddressInfo(event) {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/profile/address`)
      .update(data)
  ...
  function saveImporterBankingInfo(event) {
    ...
    importerDatabase
      .ref(`${currentUserBasePath}/profile/banking`)
      .update(data)
  ...
  function saveImporterDocumentsInfo(event) {
    event.preventDefault();
    showToast(
      "Document upload functionality will be implemented with file storage."
    );
  }
  ...
  function handleImporterPhotoUpload(event) {
    ...
    reader.onload = function (e) {
      ...
      if (currentUser && importerDatabase) {
        importerDatabase
          .ref(`${currentUserBasePath}/profile/basic/profilePhoto`)
          .set(e.target.result)
  ...
  function handleImporterDocumentUpload(event, fieldId) {
    ...
    reader.onload = function (e) {
      ...
      preview.innerHTML = `<small>${file.name} (${(file.size / 1024).toFixed(
        2
      )} KB)</small>`;
  ...
  const importerCountryDocumentMatrix = [
      ...
      requirements: [
        {
          ...
          use: "Customs valuation, duty calculation & GST compliance",
          issuer: "Foreign Exporter / Supplier",
          linkLabel: "ICEGATE Import Guidelines",
          linkUrl: "https://icegate.gov.in",
        },
        {
          document: "Packing List",
          use: "Physical examination, cargo verification & load planning",
          issuer: "Foreign Exporter / Supplier",
          linkLabel: "CBIC Import Procedures",
          linkUrl: "https://www.cbic.gov.in",
        },
        {
          document: "Bill of Entry (BOE)",
          use: "Primary import declaration & customs clearance at port",
          issuer: "Indian Importer / Customs Broker via ICEGATE",
          linkLabel: "ICEGATE Bill of Entry",
          linkUrl: "https://icegate.gov.in",
        },
        {
          document: "Bill of Lading / Air Waybill",
          use: "Proof of shipment, cargo release & title transfer",
        ...
        {
          document: "Import License / Registration",
          use: "Required for restricted/prohibited items per ITC-HS",
          issuer: "DGFT / Competent Authority",
          linkLabel: "DGFT Import Licensing",
          linkUrl: "https://dgft.gov.in",
        },
        {
          document: "Certificate of Origin",
          use: "Preferential duty claims under FTAs (ASEAN, SAFTA, etc.)",
          issuer: "Chamber of Commerce / Authorized Body in Exporting Country",
          linkLabel: "DGFT e-COO",
          linkUrl: "https://coo.dgft.gov.in",
        },
        {
          document: "Insurance Certificate",
          use: "Cargo insurance coverage during transit",
          issuer: "Insurance Company",
          linkLabel: "IRDA Insurance",
          linkUrl: "https://www.irdai.gov.in",
        },
        {
          document: "GST Registration (GSTIN)",
          use: "GST compliance & input tax credit for importer",
          issuer: "GST Department",
          linkLabel: "GST Portal",
          linkUrl: "https://www.gst.gov.in",
        },
      ],
    },
    {
      ...
      name: "United States",
      corridor:
        "Ports: New York/New Jersey, Los Angeles/Long Beach, Savannah, Houston",
      modes: ["sea", "air", "imports"],
      quickChecklist: [
      ...
      requirements: [
        {
          document: "Commercial Invoice",
          use: "CBP customs entry, duty assessment & valuation",
          issuer: "Foreign Exporter / Supplier",
          linkLabel: "US CBP Import Requirements",
          linkUrl: "https://www.cbp.gov/trade/basic-import-export",
        },
        {
          document: "Packing List",
          use: "CBP inspection, cargo examination & deconsolidation",
          issuer: "Foreign Exporter / Supplier",
          linkLabel: "CBP Trade Documentation",
          linkUrl: "https://www.cbp.gov/trade",
        },
        {
          document: "Bill of Lading / Air Waybill",
          use: "Arrival notice, cargo release & delivery order",
          issuer: "Carrier / NVOCC / Airline",
          linkLabel: "Federal Maritime Commission",
          linkUrl: "https://www.fmc.gov",
        },
        {
          document: "Importer Security Filing (ISF 10+2)",
          use: "Mandatory security filing for ocean cargo (24hrs before loading)",
          issuer: "US Importer / Customs Broker",
          linkLabel: "CBP ISF 10+2 Requirements",
          linkUrl:
            "https://www.cbp.gov/border-security/ports-entry/cargo-security/importer-security-filing-102",
        },
        {
          document: "CBP Entry (Form 3461/7501)",
          use: "Formal import clearance & duty payment via ACE",
          issuer: "US Customs Broker / Importer",
          linkLabel: "CBP ACE System",
          linkUrl: "https://www.cbp.gov/trade/automated",
        },
        {
          document: "IRS Number (EIN/SSN)",
          use: "Customs bond requirement & duty payment identification",
          issuer: "Internal Revenue Service (IRS)",
          linkLabel: "IRS EIN Application",
          linkUrl:
            "https://www.irs.gov/businesses/small-businesses-self-employed/apply-for-an-ein",
        },
        {
          document: "Customs Bond",
          use: "Guarantee for duty payment & compliance (Single/Continuous)",
          issuer: "Surety Company / Customs Broker",
          linkLabel: "CBP Customs Bonds",
          linkUrl: "https://www.cbp.gov/trade/finance/bonds",
        },
        {
          document: "FDA/USDA/Partner Agency Docs",
          use: "Regulatory clearance for food, drugs, agriculture, electronics",
          issuer: "FDA / USDA / Relevant Agency",
          linkLabel: "FDA Import Basics",
      ...
      requirements: [
        {
          ...
          use: "Customs declaration base data, VAT calculation & duty assessment",
          issuer: "Foreign Exporter / Supplier",
          linkLabel: "EU Customs & Taxation",
          linkUrl: "https://taxation-customs.ec.europa.eu",
        },
        {
          document: "Bill of Lading / Air Waybill",
          use: "Proof of shipment, cargo release at port & delivery order",
          issuer: "Carrier / Freight Forwarder",
          linkLabel: "Port of Rotterdam",
          linkUrl: "https://www.portofrotterdam.com",
        },
        {
          document: "Entry Summary Declaration (ENS/ICS2)",
          use: "Pre-arrival security/safety filing (24-48hrs before arrival)",
          issuer: "Carrier / Authorized Representative",
        ...
        },
        {
          ...
          use: "Formal import clearance, duty payment & release into free circulation",
          issuer: "EU Customs Broker / Importer",
        ...
        },
        {
          ...
          use: "Economic Operators Registration for all EU customs transactions",
          issuer: "EU Member State Customs Authority",
          linkLabel: "EORI Registration",
          linkUrl:
            "https://ec.europa.eu/taxation_customs/business/customs-procedures-import-and-export/customs-procedures/economic-operators-registration-identification-number-eori_en",
        },
        {
          document: "VAT Registration Number",
          use: "VAT payment on import & input tax credit recovery",
          issuer: "EU Member State Tax Authority",
          linkLabel: "EU VAT Information",
          linkUrl: "https://ec.europa.eu/taxation_customs/business/vat_en",
        },
        {
          document: "Certificate of Origin / EUR.1",
          use: "Preferential duty rates under EU FTAs & GSP schemes",
          issuer: "Chamber of Commerce / Competent Authority",
          linkLabel: "EU Preferential Origin",
          linkUrl:
            "https://taxation-customs.ec.europa.eu/customs-4/preferential-origins_en",
        },
        {
          document: "Product Certificates (CE, Health, Phyto)",
          use: "Compliance with EU product safety, health & phytosanitary standards",
          issuer: "Competent Authorities / Testing Labs",
          linkLabel: "EU Product Compliance",
          linkUrl: "https://ec.europa.eu/growth/single-market/ce-marking_en",
        },
      ],
    },
    {
      ...
      name: "United Kingdom",
      corridor:
        "Ports: Felixstowe, Southampton, London Gateway, Heathrow Airport",
      modes: ["sea", "air", "imports"],
      quickChecklist: [
      ...
      requirements: [
        {
          document: "Commercial Invoice & Packing List",
          use: "UK customs declaration data & HMRC assessment",
          issuer: "Foreign Exporter / Supplier",
          linkLabel: "HMRC Import Guidelines",
          linkUrl:
            "https://www.gov.uk/guidance/filling-in-your-customs-declaration",
        },
        {
          document: "Bill of Lading / Air Waybill",
          use: "Cargo release & delivery order at UK port",
          issuer: "Carrier / Freight Forwarder",
          linkLabel: "UK Border Force",
          linkUrl: "https://www.gov.uk/government/organisations/border-force",
        },
        {
          document: "UK Customs Declaration (CDS)",
          use: "Import clearance via Customs Declaration Service",
          issuer: "UK Customs Broker / Importer",
          linkLabel: "CDS Access",
          linkUrl:
            "https://www.gov.uk/guidance/get-access-to-the-customs-declaration-service",
        },
        {
          document: "EORI Number",
          use: "Economic Operators Registration for UK customs",
          issuer: "HMRC",
          linkLabel: "EORI Registration UK",
          linkUrl: "https://www.gov.uk/eori",
        },
        {
          document: "VAT Registration",
          use: "VAT payment on import & recovery",
          issuer: "HMRC",
          linkLabel: "UK VAT Registration",
          linkUrl: "https://www.gov.uk/vat-registration",
        },
        {
          document: "Certificate of Origin",
          use: "Preferential duty under UK trade agreements",
          issuer: "Chamber of Commerce",
          linkLabel: "UK Trade Agreements",
          linkUrl:
      ...
      requirements: [
        {
          ...
          use: "CBSA import valuation & duty assessment",
          issuer: "Foreign Exporter / Supplier",
        ...
        {
          document: "Packing List",
          use: "CBSA inspection & cargo examination",
          issuer: "Foreign Exporter / Supplier",
        ...
        {
          document: "Bill of Lading / Air Waybill",
          use: "Manifest & cargo release at Canadian port",
          issuer: "Carrier",
        ...
        },
        {
          ...
          use: "Formal import clearance via CARM/CCS",
          issuer: "Canadian Customs Broker / Importer",
          linkLabel: "CBSA CARM",
          linkUrl: "https://www.cbsa-asfc.gc.ca/prog/carm-gcra/menu-eng.html",
        },
        {
          document: "Business Number (BN) & Import Account",
          use: "CRA registration for duty payment & GST",
          issuer: "Canada Revenue Agency (CRA)",
          linkLabel: "CRA Business Number",
          linkUrl:
            "https://www.canada.ca/en/revenue-agency/services/tax/businesses/topics/registering-your-business/register.html",
        },
        {
          document: "CUSMA / Phytosanitary Certificates",
          use: "Preferential duty & CFIA clearance for regulated goods",
          issuer: "CFIA / Authorized Bodies",
    ...
    },
    {
      ...
      requirements: [
        {
          document: "Commercial Invoice & Packing List",
          use: "ABF import declaration & duty assessment",
          issuer: "Foreign Exporter / Supplier",
        ...
        },
        {
          ...
          use: "Formal entry (N10/N20) via ICS",
          issuer: "Australian Customs Broker / Importer",
          linkLabel: "ICS Lodgement",
          linkUrl: "https://www.abf.gov.au/help-and-support/ics",
        },
        {
          document: "ABN (Australian Business Number)",
          use: "Business registration for GST & import clearance",
          issuer: "Australian Taxation Office (ATO)",
          linkLabel: "ABN Registration",
          linkUrl: "https://www.abr.gov.au/business-super-funds/ applying-abn",
        },
        {
          document: "Biosecurity Certificates",
          use: "DAFF clearance for agriculture, food & plant products",
    ...
    },
    {
      ...
      corridor:
        "Ports: Jebel Ali, Khalifa Port, Dubai Airport, Abu Dhabi Airport",
      modes: ["sea", "air", "imports"],
      quickChecklist: [
      ...
      requirements: [
        {
          document: "Commercial Invoice & Packing List",
          use: "UAE customs valuation & inspection",
          issuer: "Foreign Exporter / Supplier",
        ...
        },
        {
          ...
          use: "Single window clearance via Dubai Trade",
          issuer: "UAE Importer / Customs Broker",
        ...
        {
          document: "Insurance Certificate",
          use: "Cargo insurance for transit coverage",
          issuer: "Insurance Company",
          linkLabel: "UAE Insurance Authority",
          linkUrl: "https://www.ia.gov.ae",
        },
      ],
    },
    {
      ...
      requirements: [
        {
          document: "Commercial Invoice & Packing List",
          use: "GACC customs clearance & duty assessment",
          issuer: "Foreign Exporter / Supplier",
        ...
        {
          document: "Certificate of Origin",
          use: "Preferential duty under China FTAs",
          issuer: "Chamber of Commerce",
          linkLabel: "China FTA Network",
          linkUrl: "http://fta.mofcom.gov.cn",
        },
      ],
    },
  ];
  ...
  function getImporterDocumentTypeInfo(docType, fallbackName = "") {
    return (
      importerDocumentTypeMap[docType] || {
  ...
  function getImporterMimeType(base64String) {
    ...
    if (
      ...
      base64String.startsWith("iVBORw0KGgoAAAA")
    ) {
      return "image/jpeg";
  ...
  function createImporterDocumentRecord({
    ...
    const { base64, mimeType: extractedMime } = extractImporterBase64Components(
      rawData,
      fileType
    );
    ...
    const resolvedMime =
      fileType ||
      extractedMime ||
      (getImporterMimeType(base64) !== "unknown"
        ? getImporterMimeType(base64)
        : "application/octet-stream");

    const record = {
      id: `${source}-${docType}-${uploadedAt || Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 7)}`,
      type: docType,
  ...
  function collectImporterRealtimeDocuments(userData = {}) {
    ...
    Object.keys(userData.documents).forEach((docKey) => {
      ...
      const normalizedValue =
        typeof rawValue === "object" && rawValue.data
          ? rawValue.data
          : rawValue;
      const fileName =
      ...
      const uploadedAt = resolveImporterUploadedAt(
        (typeof rawValue === "object" && rawValue.uploadedAt) ||
          userData[`${docKey}UpdatedAt`]
      );
  ...
  function collectImporterEKYCDocuments(ekycData = {}) {
    ...
    Object.entries(ekycData.documents).forEach(([docType, docValue]) => {
      ...
      const rawData =
        docValue.dataUrl || docValue.base64 || docValue.data || "";
      const uploadedAt = resolveImporterUploadedAt(
        docValue.uploadedAt,
        ekycData.updatedAt,
        ekycData.timestamp
  ...
  async function loadImporterDocumentsFromFirebase() {
    ...
    try {
      const snapshot = await importerDatabase
        .ref(`${currentUserBasePath}`)
        .once("value");
      const userData = snapshot.val() || {};
      ...
      if (userData.profilePhoto) {
        ...
        if (profilePhotoData) {
          const record = createImporterDocumentRecord({
            ...
            fileType:
              (typeof userData.profilePhoto === "object" &&
                userData.profilePhoto.fileType) ||
              "",
  ...
  function renderImporterDocuments(documents) {
    ...
    gridEl.innerHTML = documents
      .map((doc) => {
        ...
        const previewHTML =
          isImporterImageMimeType(doc.mimeType) && doc.data
  ...
  function updateImporterDocumentsStats(documents) {
    ...
    const totalSize = documents
      .reduce((sum, doc) => sum + parseFloat(doc.size), 0)
      .toFixed(2);
    ...
    if (totalSizeEl)
      totalSizeEl.textContent = `${(totalSize / 1024).toFixed(2)} MB`;
  ...
  function handleImporterDocumentSearch(event) {
    ...
    const filtered = importerDocumentsCache.filter(
      (doc) =>
  ...
  function populateImporterDocumentModeOptions(selectEl) {
    if (!selectEl) return;
    const options = ["all", ...IMPORTER_DOCUMENT_MODES].map(
      (mode) =>
        `<option value="${mode}">${formatImporterModeLabel(mode)}</option>`
    );
  ...
  function updateImporterDocumentRequirementsDisplay(
    presetCountryId,
    presetMode
  ) {
    const countrySelect = document.getElementById("importerDocCountrySelect");
    ...
    const targetCountryId =
      presetCountryId ||
      countrySelect?.value ||
      importerCountryDocumentMatrix[0]?.id;
  ...
  function getImporterCountryDataById(countryId) {
    return importerCountryDocumentMatrix.find(
  ...
  function renderImporterDocumentRequirementsTable(countryData, mode) {
    ...
    if (mode && mode !== "all" && !countryData.modes.includes(mode)) {
      container.innerHTML = `<div class="doc-empty-state">We haven't mapped ${formatImporterModeLabel(
        mode
    ...
    container.innerHTML = `
            <div class="doc-table-card">
                <div class="doc-table-card-head">
                    ...
                    <span class="doc-table-badge">${formatImporterModeLabel(
                      mode
                    )}</span>
                </div>
  ...
  function generateImporterDocumentChecklist() {
    ...
    if (selectedMode !== "all" && !countryData.modes.includes(selectedMode)) {
      checklistEl.innerHTML = `We have not mapped ${formatImporterModeLabel(
        selectedMode
    ...
    checklistEl.innerHTML = `
            <h4>Checklist for ${countryData.name} (${formatImporterModeLabel(
      selectedMode
    )})</h4>
            <ul>${listHtml}</ul>
  ...
  function initializeImporterDocumentsModule() {
    ...
    if (documentSearch) {
      documentSearch.addEventListener(
        "input",
        debounceImporter(handleImporterDocumentSearch, 300)
      );
    }
  ...
  async function loadMarketplaceProducts() {
    if (!exporterDatabase) {
      console.error(
        "Exporter database not initialized. Attempting to initialize..."
      );
      ...
      try {
        ...
        if (emptyEl) {
          emptyEl.innerHTML =
            '<p style="color: #ef4444;">Failed to connect to marketplace. Please refresh the page.</p>';
    ...
    try {
      ...
      try {
        const catalogSnapshot = await exporterDatabase
          .ref("productCatalog")
          .once("value");
        ...
        Object.entries(catalogData).forEach(([productId, product]) => {
          if (product && product.name && product.name.trim() !== "") {
            ...
            const priceValue =
              parseFloat(product.priceValue) || parseFloat(product.price) || 0;
            const priceCurrency =
            ...
            if (product.imageBase64) {
              if (product.imageBase64.startsWith("data:")) {
                ...
                const match = product.imageBase64.match(
                  /^data:([^;]+);base64,/
                );
                if (match) {
            ...
            if (!normalizedProduct.imageFileType) {
              if (product.imageFileType) {
                ...
                normalizedProduct.imageBase64 &&
                !normalizedProduct.imageBase64.startsWith("data:")
              ) {
        ...
        console.log(
          ...
          } products from productCatalog, added ${
            allProducts.length
          } valid products`
        );
      } catch (catalogError) {
        ...
        Object.entries(usersData).forEach(([userId, userData]) => {
          if (userData && userData.products) {
            ...
            const productsArray = Array.isArray(userProducts)
              ...
              : Object.entries(userProducts).map(([productId, product]) => ({
                  id: productId,
                  ...product,
                }));
              ...
              if (product && product.name && product.name.trim() !== "") {
                ...
                if (!normalizedProduct.imageFileType) {
                  if (product.imageFileType) {
                    ...
                    const ext = product.imageFileName
                      .split(".")
                      .pop()
                      ?.toLowerCase();
                    if (ext === "jpg" || ext === "jpeg") {
                ...
                const existingIndex = allProducts.findIndex(
                  (p) =>
                    (p.id === normalizedProduct.id &&
                      p.exporterId === userId) ||
                    (p.name === normalizedProduct.name &&
                      p.exporterId === userId)
                );

                // Only add if not draft and doesn't already exist
                if (
                  normalizedProduct.status !== "draft" &&
                  existingIndex === -1
                ) {
                  allProducts.push(normalizedProduct);
                } else {
      ...
      console.log(
        `Valid products after filtering: ${validProducts.length} out of ${allProducts.length}`
      );
  ...
  async function addToImporterCart(productId, exporterId) {
    ...
    try {
      ...
      let productSnapshot = await exporterDatabase
        .ref(`productCatalog/${productId}`)
        .once("value");
      let product = productSnapshot.val();

      // If not found in catalog, try users/{exporterId}/products/{productId}
      if (!product && exporterId) {
        productSnapshot = await exporterDatabase
          .ref(`users/${exporterId}/products/${productId}`)
          .once("value");
      ...
      const cartPath = currentUserBasePath
        ? `${currentUserBasePath}/cart`
        : `importers/${currentUser.uid}/cart`;
      ...
      const existingItem = cart.find(
        (item) =>
          item.id === productId &&
          item.exporterId === normalizedProduct.exporterId
      );
      ...
      .map((item) => {
        ...
        if (item.imageBase64) {
          if (item.imageBase64.startsWith("data:")) {
            ...
            imageSrc = `data:${item.imageFileType || "image/jpeg"};base64,${
              item.imageBase64
            }`;
          }
        } else if (item.image) {
        ...
        // Handle price - support both price/priceValue and currency/priceCurrency
        const itemPrice =
          parseFloat(item.priceValue) || parseFloat(item.price) || 0;
        ...
        <div class="cart-item" style="display: flex; gap: 1rem; padding: 1rem; background: #f9f9f9; border-radius: 12px; margin-bottom: 1rem;">
          ...
          item.name
        )}" style="width: 100px; height: 100px; object-fit: cover; border-radius: 8px; flex-shrink: 0;" onerror="this.src='https://via.placeholder.com/100x100?text=No+Image'">
            <div style="flex: 1; display: flex; flex-direction: column; gap: 0.5rem;">
                <div style="font-weight: 600; color: #333; font-size: 1rem;">${escapeHtml(
                  item.name || "Unnamed Product"
                )}</div>
                <div style="color: #667eea; font-weight: 600; font-size: 1.1rem;">${escapeHtml(
                  itemCurrency
                )} ${totalPrice.toLocaleString()}</div>
                ${
                  item.exporterName
                    ? `<div style="color: #666; font-size: 0.9rem;">From: ${escapeHtml(
                        item.exporterName
                      )}</div>`
                    : ""
                }
                <div style="display: flex; align-items: center; gap: 1rem; margin-top: auto;">
                    <div style="display: flex; align-items: center; gap: 0.5rem; background: white; border: 2px solid #e0e0e0; border-radius: 6px; padding: 0.25rem 0.5rem;">
                                <button onclick="updateImporterCartQuantity('${
                                  item.id
                                }', '${
          item.exporterId
        }', -1)" style="background: none; border: none; cursor: pointer; color: #667eea; font-weight: 600; padding: 0.25rem 0.5rem;">-</button>
          ...
          item.exporterId
                            ...
                            <button onclick="removeImporterCartItem('${
                              item.id
                            }', '${
          item.exporterId
        }')" style="background: #ff4757; color: white; border: none; border-radius: 6px; padding: 0.5rem 1rem; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                        Remove
                    </button>
                </div>
            </div>
  ...
  async function updateImporterCartQuantity(productId, exporterId, change) {
    ...
    try {
      const snapshot = await importerDatabase
        .ref(`users/${currentUser.uid}/cart`)
        .once("value");
      const cartData = snapshot.val();
      let cart = cartData
        ? Array.isArray(cartData)
      ...
      const item = cart.find(
        (item) => item.id === productId && item.exporterId === exporterId
      );
      if (!item) return;
  ...
  function openCheckoutModal(cart) {
    ...
    if (currentUser) {
      const profilePath = currentUserBasePath
        ? `${currentUserBasePath}/profile`
        : `importers/${currentUser.uid}/profile`;
      importerDatabase
        .ref(profilePath)
        .once("value")
        .then((snapshot) => {
          ...
          if (fullNameEl && profile.fullName)
            fullNameEl.value = profile.fullName;
          if (emailEl && currentUser.email) emailEl.value = currentUser.email;
          if (phoneEl && profile.phoneNumber)
            phoneEl.value = profile.phoneNumber;
          if (addressEl && profile.businessAddress)
            addressEl.value = profile.businessAddress;
        ...
        .map((item) => {
          ...
          return `
                    <div class="order-summary-item">
                        <span>${escapeHtml(item.name || "Unnamed Product")} x ${
            item.quantity || 1
          }</span>
  ...
  function selectPaymentMethod(method) {
    ...
    if (razorpayContainer) {
      razorpayContainer.style.display =
        method === "razorpay" ? "block" : "none";
    }
    if (payLaterContainer) {
      payLaterContainer.style.display =
        method === "paylater" ? "block" : "none";
    }
  }
  ...
  function handleCheckoutFileUpload(event, inputId) {
    ...
    reader.onload = (e) => {
      ...
      previewEl.textContent = `✓ ${file.name} (${(file.size / 1024).toFixed(
        2
      )} KB)`;
  ...
  function validateCheckoutData() {
    ...
    if (
      ...
      showCheckoutMessage(
        "Please upload required documents (Commercial Invoice and Packing List).",
        "error"
      );
      goToCheckoutStep(1);
      return false;
  ...
  function getCheckoutFormData() {
    ...
    const currency =
      currentCheckoutCart[0]?.priceCurrency ||
      currentCheckoutCart[0]?.currency ||
      "USD";
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        try {
          ...
          showCheckoutMessage(
            "Payment successful but error saving order. Please contact support.",
            "error"
          );
        }
      },
  ...
  async function saveImporterOrder(orderData) {
    try {
      ...
      // Use importers path to match security rules
      const ordersPath = currentUserBasePath
        ? `${currentUserBasePath}/orders`
        : `importers/${currentUser.uid}/orders`;
      ...
      for (const [exporterId, exporterOrder] of Object.entries(
        ordersByExporter
      )) {
        try {
          const orderRef = exporterDatabase
            .ref(`users/${exporterId}/incomingOrders`)
            .push();
  ...
  function renderImporterOrderHistory(orders) {
    ...
    if (orders.length === 0) {
      historyEl.innerHTML =
        '<p style="text-align: center; color: #64748b; padding: 2rem;">No orders yet</p>';
      return;
    }

    historyEl.innerHTML = orders
      .map((order) => {
        const statusClass =
        ...
        return `
                <div class="order-history-item" style="margin-bottom: 1.5rem; padding: 1.5rem; background: white; border-radius: 12px; border: 1px solid #e2e8f0;">
                    <div class="order-history-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 1px solid #e2e8f0;">
                        <span class="order-id" style="font-weight: 600; color: #334155;">Order #${
                          order.orderId || order.id
                        }</span>
                        <span class="order-status ${statusClass}" style="padding: 0.25rem 0.75rem; border-radius: 6px; font-size: 0.85rem; font-weight: 500; background: ${
          ...
          statusClass === "completed"
            ...
            : "#92400e"
                    ...
                    <div class="order-details" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
                        <div><strong style="color: #64748b;">Total:</strong> <span style="color: #334155;">${
                          order.currency || "USD"
                        } ${
          order.totalAmount?.toLocaleString() || "0"
        }</span></div>
                        <div><strong style="color: #64748b;">Items:</strong> <span style="color: #334155;">${
                          order.items?.length || 0
                        }</span></div>
                        <div><strong style="color: #64748b;">Payment:</strong> <span style="color: #334155;">${
                          order.paymentMethod || "N/A"
                        }</span></div>
                        <div><strong style="color: #64748b;">Date:</strong> <span style="color: #334155;">${new Date(
                            ...
                            ${order.orderUpdates
                              .map(
                                ...
                                <div style="padding: 1rem; background: #f0f9ff; border-radius: 8px; border-left: 3px solid #0ea5e9;">
                                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                                        <span style="font-size: 0.85rem; color: #64748b;">${new Date(
                                          update.sentAt || update.createdAt
                                        ).toLocaleString()}</span>
                                        ${
                                          update.sentByName
                                            ? `<span style="font-size: 0.85rem; color: #64748b;">From: ${escapeHtml(
                                                update.sentByName
                                              )}</span>`
                                            : ""
                                        }
                                    ...
                                    ${
                                      update.message
                                        ? `<p style="color: #334155; margin: 0.5rem 0;">${escapeHtml(
                                            update.message
                                          )}</p>`
                                        : ""
                                    }
                                                ...
                                                ${update.documents
                                                  .map((doc) => {
                                                    ...
                                                    return `
                                                        <a href="${docUrl}" download="${escapeHtml(
                                                      doc.fileName || "document"
                                                    )}" target="_blank" style="display: inline-flex; align-items: center; gap: 0.25rem; padding: 0.5rem 0.75rem; background: #0ea5e9; color: white; border-radius: 6px; text-decoration: none; font-size: 0.85rem;">
                                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                                                <polyline points="7 10 12 15 17 10"></polyline>
                                                                <line x1="12" y1="15" x2="12" y2="3"></line>
                    ...
                    ${
                      ...
                      order.status !== "completed"
                        ...
                        <div class="order-actions" style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #e2e8f0;">
                            <button class="btn-cancel-order" onclick="cancelImporterOrder('${
                              order.orderId || order.id
                            }')" style="padding: 0.5rem 1rem; border-radius: 6px; border: 1px solid #ef4444; background: white; color: #ef4444; cursor: pointer;">
                                Cancel Order
                            </button>
                        </div>
                    `
  ...
  async function cancelImporterOrder(orderId) {
    ...
    try {
      ...
      if (Array.isArray(ordersData)) {
        ordersData.forEach((order, index) => {
          if (order.orderId === orderId) {
            ...
            updates[`${ordersPath}/${index}/updatedAt`] =
        ...
        Object.keys(ordersData).forEach((key) => {
          if (ordersData[key].orderId === orderId) {
            ...
            updates[`${ordersPath}/${key}/updatedAt`] =
              new Date().toISOString();
            orderFound = true;
          }
        });
      ...
      const orderSnapshot = await importerDatabase
        .ref(ordersPath)
        .once("value");
      ...
      if (order && order.items) {
        ...
        const exporters = [
          ...new Set(
            order.items.map((item) => item.exporterId).filter(Boolean)
          ),
        ];

        for (const exporterId of exporters) {
          try {
            const exporterOrdersSnapshot = await exporterDatabase
              .ref(`users/${exporterId}/incomingOrders`)
              .once("value");
            ...
            if (exporterOrders) {
              ...
              Object.keys(exporterOrders).forEach((key) => {
                if (exporterOrders[key].orderId === orderId) {
                  exporterUpdates[
                    `users/${exporterId}/incomingOrders/${key}/status`
                  ] = "cancelled";
                  exporterUpdates[
                    `users/${exporterId}/incomingOrders/${key}/updatedAt`
                  ] = new Date().toISOString();
                }
              });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const resultsSection = document.getElementById(
    "importerForwardContractResults"
  );
  const currencyPairSelect = document.getElementById(
    "importerForwardContractCurrencyPair"
  );
  const currencyPriceDiv = document.getElementById(
    "importerForwardContractCurrencyPrice"
  );
  ...
  async function generateImporterPDF(data) {
    ...
    const importerAuth = window.getImporterAuth
      ? window.getImporterAuth()
      : null;
    const importerDatabase = window.getImporterDatabase
      ? window.getImporterDatabase()
      : null;
    const currentUserBasePath = window.getCurrentUserBasePath
      ? window.getCurrentUserBasePath()
      : "";

    if (importerAuth && importerDatabase && importerAuth.currentUser) {
      const user = importerAuth.currentUser;
      const contractId = `contract_${new Date().getTime()}_${Math.random()
        .toString(36)
        .slice(2, 9)}`;
      ...
      try {
        const contractsPath = currentUserBasePath
          ? `${currentUserBasePath}/forwardContracts/${contractId}`
          : `importers/${user.uid}/forwardContracts/${contractId}`;
        ...
        successMsg.style.cssText =
          "margin-top: 15px; padding: 12px; background: #d1fae5; color: #065f46; border-radius: 8px; font-size: 14px;";
        successMsg.textContent =
          "✅ Contract saved successfully! You can view it in the Payments section.";
        resultsSection
          .querySelector(".forward-contract-result-block")
        ...
        errorMsg.style.cssText =
          "margin-top: 15px; padding: 12px; background: #fee2e2; color: #991b1b; border-radius: 8px; font-size: 14px;";
        errorMsg.textContent =
          "⚠️ Contract saved locally but failed to save to database. Please try again.";
        resultsSection
          .querySelector(".forward-contract-result-block")
      ...
      console.warn(
        "Firebase not initialized or user not logged in. Contract saved locally only."
      );
    }
  }
  ...
  async function loadImporterForwardContractsInPayments() {
    const contractsList = document.getElementById(
      "importerForwardContractsList"
    );
    const contractsEmpty = document.getElementById(
      "importerForwardContractsEmpty"
    );
    ...
    if (!importerAuth || !importerDatabase || !importerAuth.currentUser) {
      contractsList.innerHTML =
        '<div style="text-align: center; padding: 40px; color: #6b7280;">Please log in to view forward contracts.</div>';
      return;
    }
    ...
    try {
      contractsList.innerHTML =
        '<div class="forward-contracts-loading" style="text-align: center; padding: 40px;"><div class="spinner" style="border: 3px solid #f3f3f3; border-top: 3px solid #3498db; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto 20px;"></div><p>Loading forward contracts...</p></div>';

      const contractsPath = currentUserBasePath
        ? `${currentUserBasePath}/forwardContracts`
        : `importers/${user.uid}/forwardContracts`;
      ...
      const contractsArray = Object.keys(contracts)
        .map((key) => ({
          id: key,
          ...contracts[key],
        }))
      ...
      contractsList.innerHTML =
        '<div style="text-align: center; padding: 40px; color: #dc2626;">Error loading forward contracts. Please try again.</div>';
    }
  }
  ...
  function createImporterForwardContractCard(contract) {
    ...
    const currentRate = contract.currentRate
      ? contract.currentRate.toFixed(4)
      : "N/A";
    const bestTradeDate =
      contract.bestTradeDay && contract.bestTradeDay.date
        ...
        : "N/A";
    const bestTradePrice =
      contract.bestTradeDay && contract.bestTradeDay.price
    ...
    card.innerHTML = `
            <div class="forward-contract-card-header">
                <div>
                    <div class="forward-contract-card-title">Forward Contract - ${
                      contract.currencyPair
                    }</div>
                    <div class="forward-contract-card-date">Created: ${formattedDate}</div>
                </div>
                <span class="forward-contract-card-badge">${
            ...
            <div class="forward-contract-card-details">
                <div class="forward-contract-detail-item">
                    ...
                    <span class="forward-contract-detail-value">${formatAmount(
                      contract.amount
                    )}</span>
                </div>
                <div class="forward-contract-detail-item">
                    <span class="forward-contract-detail-label">Currency Pair</span>
                    <span class="forward-contract-detail-value">${
                      contract.currencyPair
                    }</span>
                </div>
                <div class="forward-contract-detail-item">
            ...
            <div class="forward-contract-card-actions">
                <button class="forward-contract-action-btn forward-contract-action-btn-primary" onclick="downloadImporterForwardContractPDF('${
                  contract.id
                }')">
                    <span>📄</span>
                    <span>Download PDF</span>
                </button>
                <button class="forward-contract-action-btn forward-contract-action-btn-secondary" onclick="viewImporterForwardContractDetails('${
                  contract.id
                }')">
                    <span>👁️</span>
  ...
  async function downloadImporterForwardContractPDF(contractId) {
    ...
    try {
      ...
      link.href = url;
      link.download =
        contract.pdfFileName || `Forward_Contract_${contractId}.pdf`;
    ...
    try {
      ...
      const bestTradePrice =
        contract.bestTradeDay && contract.bestTradeDay.price
          ? `₹${contract.bestTradeDay.price.toFixed(2)}`
          : "N/A";
      const bestTradeReason =
        contract.bestTradeDay && contract.bestTradeDay.reason
...
Created: ${new Date(contract.createdAt).toLocaleString()}
  ...
  // Make functions globally available
  window.downloadImporterForwardContractPDF =
    downloadImporterForwardContractPDF;
  window.viewImporterForwardContractDetails =
    viewImporterForwardContractDetails;
});
