...
(function () {
  ...
  function initHeroAnimation() {
    ...
    logoCenter.addEventListener("click", function (e) {
      ...
      console.log(
        "Logo clicked - Current state: isFixed=" +
          isFixed +
          ", isExpanded=" +
          isExpanded
      );
  ...
  function initButtonHandlers() {
    ...
    document
      .querySelectorAll(".btn-primary, .btn-get-started")
      .forEach((button) => {
...
function initializeFirebaseApp(config, name) {
  try {
    ...
    console.warn(
      `Firebase initialization error for ${
        name || "default app"
...
firebaseApps.importer = initializeFirebaseApp(
...
async function handleEmailLogin(event, userType = currentUserType) {
  ...
  if (!auth) {
    ...
    showAuthMessage(
      `Firebase is not configured for ${userType}. Please add your Firebase credentials.`,
      "error"
    );
    return;
  }
  ...
  if (!firebase || !firebase.apps || firebase.apps.length === 0) {
    ...
    showAuthMessage(
      "Firebase is not initialized. Please refresh the page and try again.",
      "error"
    );
    return;
  }
  ...
  const buttonId =
    userType === "importer" ? "importerEmailLoginBtn" : "emailLoginBtn";
    ...
    if (!otpSent) {
      ...
      if (emailOrPhone.includes("@")) {
        ...
        const phoneNumber = emailOrPhone.startsWith("+")
          ? emailOrPhone
          : `+${emailOrPhone}`;
        otpConfirmationResult = await auth.signInWithPhoneNumber(
          phoneNumber,
          recaptchaVerifier
        );
...
async function handleForgotPassword(event, userType = currentUserType) {
  ...
  const emailInputId =
    userType === "importer" ? "importerLoginEmail" : "loginEmail";
